...
def get_attribute_to_properties():
    """
    ...
    attribute_dict = {
        'format' : ['yymmddhhmnss', 'yyyy-mm-dd'],
        'mode' : ['<debug mode:', 'auto', 'normal', 'off', '<file mode:', 'append', 'read', 'write'],
    ...
    # Extend `global_attributes` by the event handler attributes
    global_properties.extend([
    ])

    for properties in attribute_dict.values():
...
class XpxTagCompletions(sublime_plugin.EventListener):
    ...
    def __init__(self):
        ...
        self.attribute_to_properties = get_attribute_to_properties()
    ...
    def get_values_completions(self, view, pt, prefix):
        ...
        properties = self.attribute_to_properties.get(myAttributeName, [])
        # ("class\tAttr", "class"),
        attri_completions = [(a + '\tPropertie', a) for a in properties]
        return attri_completions
